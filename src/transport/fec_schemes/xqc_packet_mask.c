...
void xqc_lookup_pkm(xqc_connection_t *conn, xqc_int_t src_symbol_num, xqc_int_t rpr_symbol_num, uint8_t *output)
{
...
void xqc_get_packet_mask(xqc_connection_t *conn, xqc_int_t src_symbol_num, xqc_int_t rpr_symbol_num, uint8_t *tbl)
{
...
void xqc_packet_mask_init_one(xqc_connection_t *conn, uint8_t bm_idx)
{
...
/**
 * TODO：fec: maybe need a function to update block size/ code rate,
                           ...
                           xqc_int_t block_id, xqc_int_t symbol_idx)
{
    ...
    xqc_list_for_each_safe(pos, next, src_list)
    {
        ...
        if (src_block_id == block_id && *(rpr_symbol->recv_mask + src_mask_offset) & (1 << (7 - src_symbol_idx % 8)))
        {
