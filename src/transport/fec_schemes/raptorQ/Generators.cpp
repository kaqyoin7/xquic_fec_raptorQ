...
using namespace std;
// 核心的编码/解码矩阵生成与操作逻辑
// 主要功能：参数初始化、元组生成、矩阵构建（GLDPC、GHDPC、GLT）、高斯消元、修复符号生成、中间符号生成、符号恢复等
// 直接操作符号数据和生成矩阵，支撑编码器和解码器的底层实现,包含大量与 RaptorQ 算法相关的数学和矩阵操作
// #define DEBUG 1
...
void Generators::_3_Matrix_GHDPC()
{
	...
	for (i = S; i < S + H; i++)
	{
		for (j = 0; j < K1 + S; j++)
		{
			...
			for (k = j; k < K1 + S; k++)
				if (A[i][k].val)
					tmp ^= octmul(A[i][k].val, OCT_EXP[k - j]);
			A[i][j].val = tmp;
		}
	}
