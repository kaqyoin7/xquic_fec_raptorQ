...
xqc_set_valid_encoder_scheme_cb(xqc_fec_code_callback_t *callback, xqc_int_t scheme)
{
    ...
    case XQC_PACKET_MASK_CODE:
        ...
        return XQC_OK;
#endif
#ifdef XQC_ENABLE_FOUNTAIN
    case XQC_RAPTORQ_CODE:
        callback->xqc_fec_init = xqc_fountain_code_cb.xqc_fec_init;
        callback->xqc_fec_init_one = xqc_fountain_code_cb.xqc_fec_init_one;
        callback->xqc_fec_encode = xqc_fountain_code_cb.xqc_fec_encode;
        return XQC_OK;
#endif
    }
...
#ifdef XQC_ENABLE_FOUNTAIN
    case XQC_RAPTORQ_CODE:
        callback->xqc_fec_decode = xqc_fountain_code_cb.xqc_fec_decode;
        return XQC_OK;
#endif
    }
    ...
    *fec_schemes_buff_len = 0;
    for (i = 0, j = 0; i < schemes_len && j < XQC_FEC_MAX_SCHEME_NUM; i++)
    {
        ...
        case XQC_PACKET_MASK_CODE:
            ...
            break;
        case XQC_RAPTORQ_CODE:
            fec_schemes_buff[j] = XQC_RAPTORQ_CODE;
            j++;
            break;
        default:
            break;
...
{
    if (packet_out->po_flag & XQC_POF_USE_FEC && packet_out->po_frame_types & conn->conn_settings.fec_params.fec_protected_frames)
    {
        return XQC_OK;
    }
...
void xqc_stream_fec_init(xqc_stream_t *stream)
{
...
void xqc_init_src_symbol_value(xqc_fec_src_syb_t *symbol)
{
    symbol->block_id = 0;
    symbol->symbol_idx = 0;
...
void xqc_init_rpr_symbol_value(xqc_fec_rpr_syb_t *symbol)
{
    symbol->block_id = 0;
    symbol->symbol_idx = 0;
...
void xqc_remove_src_symbol_from_list(xqc_fec_ctl_t *fec_ctl, xqc_fec_src_syb_t *src_symbol)
{
...
xqc_negotiate_fec_schemes(xqc_connection_t *conn, xqc_transport_params_t params)
{
    ...
    if (ls->enable_encode_fec && params.enable_decode_fec)
    {
        ...
        if (conn->conn_type == XQC_CONN_TYPE_CLIENT && params.fec_decoder_schemes_num > 1)
        {
...
set_decoder:
    ...
    {
        // server should provide only 1 scheme if negotiation success;
        if (conn->conn_type == XQC_CONN_TYPE_CLIENT && params.fec_encoder_schemes_num > 1)
        {
        ...
        printf("|set final decoder fec scheme: %s", xqc_get_fec_scheme_str(conn->conn_settings.fec_params.fec_decoder_scheme));
        ret = XQC_OK;
    }
end:
...
void xqc_update_rpr_symbol_mask_on_src(xqc_list_head_t *head, xqc_int_t block_id,
                                       xqc_int_t pi_sym_idx)
{
    ...
    xqc_list_for_each_safe(pos, next, head)
    {
        ...
        if (symbol->block_id == block_id && *(symbol->repair_key + mask_offset) & (1 << (7 - pi_sym_idx % 8)))
        {
...
void xqc_update_rpr_symbol_mask_on_rpr(xqc_list_head_t *head, xqc_fec_rpr_syb_t *rpr_symbol)
{
    xqc_list_head_t *pos, *next;
    ...
    xqc_list_for_each_safe(pos, next, head)
    {
        ...
        if (block_id == rpr_symbol->block_id && *(rpr_symbol->repair_key + mask_offset) & (1 << (7 - symbol_idx % 8)))
        {
                       ...
                       unsigned char *symbol, xqc_int_t symbol_size)
{
    ...
    if (block_id != 0 && !xqc_if_src_blk_exists(conn->fec_ctl, block_id) && block_id <= conn->fec_ctl->fec_max_fin_blk_id)
    {
        return -XQC_EFEC_TOLERABLE_ERROR;
    }

    if (conn->fec_ctl->fec_src_syb_num > window_size && !xqc_if_src_blk_exists(conn->fec_ctl, block_id))
    {
...
xqc_process_rpr_symbol(xqc_connection_t *conn, xqc_fec_rpr_syb_t *tmp_rpr_symbol)
{
    ...
    if (conn->fec_ctl->fec_rpr_syb_num > window_size && !xqc_if_rpr_blk_exists(conn->fec_ctl, block_id))
    {
...
void xqc_fec_on_stream_size_changed(xqc_stream_t *quic_stream)
{
    ...
    if (conn->fec_ctl->fec_send_required_repair_num[0] != rpr_syb_num || conn->conn_settings.fec_params.fec_max_symbol_num_per_block != src_syb_num)
    {
