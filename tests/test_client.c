...
{
    xqc_engine_t *engine;    // QUIC 引擎实例
    struct event *ev_engine; // 用于定时驱动 QUIC 引擎的 libevent 定时器
    int log_fd;              // 日志文件描述符
    int keylog_fd;           // SSL/TLS Keylog 文件描述符
    struct event *ev_delay;  // 控制连接延迟（可用于延迟连接启动）的定时器事件
    struct event_base *eb;   // libevent 的事件主循环上下文
    struct event *ev_conc;   // 控制并发连接的定时器事件
    int cur_conn_num;        // 当前正在活跃的连接数
} client_ctx_t;
...
int main(int argc, char *argv[])
{
    // 全局变量初始化
    g_req_cnt = 0;                         // 已完成请求数
    g_bytestream_cnt = 0;                  // bytestream请求数
    g_req_max = 1;                         // 总请求数，默认1
    g_send_body_size = 1024 * 1024;        // 发送body大小，默认1MB
    g_send_body_size_defined = 0;          // 是否手动指定body大小
    g_send_body_size_from_cdf = 0;         // 是否从cdf文件读取body大小
    cdf_list_size = 0;                     // cdf列表大小
    cdf_list = NULL;                       // cdf列表指针
    g_save_body = 0;                       // 是否保存响应body到文件
    g_read_body = 0;                       // 是否从文件读取发送body
    g_echo_check = 0;                      // 是否开启回显校验
    g_drop_rate = 0;                       // 丢包率（‰）
    g_spec_url = 0;                        // 是否指定url
    g_is_get = 0;                          // 是否GET请求，默认POST
    g_test_case = 0;                       // 测试用例编号
    g_ipv6 = 0;                            // 是否使用IPv6
    g_no_crypt = 0;                        // 是否禁用加密
    g_max_dgram_size = 0;                  // datagram最大长度
    g_send_dgram = 0;                      // 是否发送datagram
    g_req_paral = 1;                       // 每连接并发请求数
    g_copa_ai = 1.0;                       // copa算法参数
    g_copa_delta = 0.05;                   // copa算法参数
    g_dgram_qos_level = XQC_DATA_QOS_HIGH; // datagram qos等级
    g_pmtud_on = 0;                        // 是否启用PMTUD

    char server_addr[64] = TEST_SERVER_ADDR; // 服务器地址
    g_server_addr = server_addr;
    int server_port = TEST_SERVER_PORT;          // 服务器端口
    char c_cong_ctl = 'b';                       // 拥塞控制算法，默认bbr
    char c_log_level = 'd';                      // 日志级别，默认debug
    int c_cong_plus = 0;                         // bbr/bbr2增强开关
    int pacing_on = 0;                           // 是否启用pacing
    int transport = 0;                           // 传输层类型，0为HTTP3
    int use_1rtt = 0;                            // 是否强制1RTT
    ...
    strcpy(g_log_path, "./clog"); // 日志路径

    srand(0); // fix the random seed 固定随机种子，便于复现
    ...
    // 设置引擎回调，包括定时器、日志、keylog
    xqc_engine_callback_t callback = {
        .set_event_timer = xqc_client_set_event_timer, /* 定时器到期时调用主逻辑 */
        .log_callbacks = {
            .xqc_log_write_err = xqc_client_write_log,
            .xqc_log_write_stat = xqc_client_write_log,
            .xqc_qlog_event_write = xqc_client_write_qlog,
    ...
    // 设置传输层回调，包括 socket 写、token/session 保存、证书校验等
    ...
    // 拷贝额外连接选项字符串（命令行 --conn_options 设置的值），用于传递某些复杂参数，例如实验功能标志或可扩展选项
    ...
    if (g_enable_fec)
    {
        ...
        // 设置fec_params.fec_encoder_schemes[0]为参数配置所选的FEC方案对应定义值，详见xquic.h中xqc_fec_schemes_e的枚举值
